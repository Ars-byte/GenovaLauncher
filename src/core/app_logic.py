...
def _ensure_mc_libraries(app):
    ...
    if mode == c.MODE_BIN_CUSTOM:
        ...
        if cl:
            candidates.append(os.path.join(os.path.dirname(cl), "..", "libs_mc", "lib", arch))
    # 2) Bundled en GenovaLauncher mismo (libs_mc/ en la raíz del proyecto)
    ...
    if os.path.isdir(downloads):
        for entry in os.listdir(downloads):
            ...
            if not os.path.isdir(full):
                continue
            # Match both old (CianovaLauncher) and new (GenovaLauncher) dirs
            if entry.startswith(("CianovaLauncher", "GenovaLauncher", "mcpelauncher")):
...
def launch_game(app):
    ...
    if app.config.get("controller_enabled", False):
        ...
        if os.path.isfile(gamepad_db):
            env["SDL_GAMECONTROLLERCONFIG_FILE"] = gamepad_db
        env["SDL_JOYSTICK_DEVICE"] = "/dev/input/js*"
        env["SDL_GAMECONTROLLER_ALLOW_STEAM"] = "1"
    if app.running_in_flatpak:
