...
def _ensure_mc_libraries(app):
    ...
    if mode == c.MODE_BIN_CUSTOM:
        ...
        if cl:
            candidates.append(os.path.join(os.path.dirname(cl), "..", "libs_mc", "lib", arch))
    # 2) Bundled en PYmc mismo (libs_mc/ en la raíz del proyecto)
    ...
    if os.path.isdir(downloads):
        for entry in os.listdir(downloads):
            ...
            if not os.path.isdir(full):
                continue
            # Match both old (CianovaLauncher) and new (PYmc) dirs
            if entry.startswith(("CianovaLauncher", "PYmc", "mcpelauncher")):
